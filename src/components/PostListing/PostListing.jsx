...
class PostListing extends React.Component {
  ...
  linkToPost(post) {
    ...
    return (<Link className={styles.link} to={post.path} key={post.title}>
      <div className={styles.insidePostContainer}>
        <div className={styles.titleAndDateContainer}>
          <h1 className={styles.title}>
            {mustRead ? <span className={styles.mustRead}>*MUST READ*<br/></span> : null}
            {post.title}
          </h1>
          <p className={styles.dateAuthor}>{post.date + " - " + post.author + " - " + post.timeToRead + "min read"}</p>
          <p className={styles.category}>{"Category: " + post.category}</p>
        </div>
        <div className={styles.coverContainer}>
            <Img className={styles.cover} fluid={post.cover.childImageSharp.fluid}/>
        </div>
