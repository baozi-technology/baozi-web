...
const FooterV1 = () => {
  ...
  const footerLogoResult = useStaticQuery(graphql`
    {
      mainFooterLogo: file(relativePath: { eq: "logos/baozi-only-logo.png" }) {
        publicURL
      }
      gifFooterLogo: file(relativePath: { eq: "logos/baozi-only-logo.gif" }) {
        publicURL
      }
    }
  `);
  const { mainFooterLogo, gifFooterLogo } = footerLogoResult;
          ...
          <div title="Scroll to top" className={styles.logoToTopContainer}>
            <a onClick={() => scrollToTop()}>
              <img
                ...
                src={!hover?mainFooterLogo.publicURL:gifFooterLogo.publicURL}
                onMouseOver={() => {
                    setHover(true);
                }}
                onMouseOut={() => {
